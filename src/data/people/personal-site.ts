 ...
 *   - Anyone with a registered profile (PROFILES) → their internal CASSA profile page.
...
import { hasProfile } from "./profiles.ts";
...
export function personalSite(
  ...
  if (hasProfile(slug)) return withBase(`/people/${slug}`);
